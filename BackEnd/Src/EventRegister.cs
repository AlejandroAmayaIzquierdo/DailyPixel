...
{
    public static HashSet<Type> FindAndInjectClientBinaryEventHandlers(
    ...
    public static void InvokeEventHandlerBinaryData(
    ...
    )
    {
        ...
        foreach (var type in types)
        {
            ...
            dynamic? clientEventService =
                serviceProvider.GetService(type)
                ?? throw new InvalidOperationException(
                    $"Could not resolve service for header: {header}"
                );
